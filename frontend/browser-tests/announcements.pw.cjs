...
test('employee dashboard and acknowledgment work on desktop and mobile', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name:'Company announcements' })).toBeVisible();
  await page.screenshot({ path:'test-results/announcements-dashboard-desktop.png', fullPage:true });
  await page.getByRole('link', { name:/A new chapter for our team/ }).click();
...
test('Super Admin management form and tracking fit mobile and dark theme', async ({ page }) => {
  await setup(page,'SUPER_ADMIN');
